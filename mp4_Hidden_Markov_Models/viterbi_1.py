    ...
    for tag in unique_tag:
        for word in unique_word:
                ...
                if temp_pair in word_tag_dict:
                        ...
                        tag_word_prob[temp_pair] = math.log((laplace_smooth_k)/ (tag_count[tag] + laplace_smooth_k*(len(unique_word)+1)))
        unseen_wt_pair = (-1, tag)
        tag_word_prob[unseen_wt_pair] = math.log((laplace_smooth_k)/ (tag_count[tag] + laplace_smooth_k*(len(unique_word)+1)))
